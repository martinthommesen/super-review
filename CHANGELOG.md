...
- Snapshot output control: `--snapshot` gains `--metadata-only` and `--out FILE` for bounded output and exact-byte capture to a file (`--out` refuses paths inside the reviewed repository and writes through a pinned directory descriptor so a concurrent parent swap cannot redirect the copy into it); the lifecycle now prefers the metadata-first flow before replacement, and an oversized or malformed prior report yields a `Partial`/`Blocked` run instead of streaming its content.
